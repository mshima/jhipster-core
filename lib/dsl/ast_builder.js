...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  applicationSubDeclaration(context) {
    ...
    if (context.applicationSubConfigWithAnnotations) {
      // Apparently the pegjs grammar only returned the last config
      applicationSubDeclaration.config = this.visit(
        context.applicationSubConfigWithAnnotations[context.applicationSubConfigWithAnnotations.length - 1]
      );
    }
  ...
  applicationSubConfigWithAnnotations(context) {
    const annotations = [];
    if (context.annotationDeclaration) {
      context.annotationDeclaration.forEach(contextObject => {
        annotations.push(this.visit(contextObject));
    ...
    if (context.applicationSubConfig) {
      // Apparently the pegjs grammar only returned the last config
      config = this.visit(context.applicationSubConfig[context.applicationSubConfig.length - 1]);
    }
    annotations.forEach(annotation => {
      config[annotation.optionName] = annotation.optionValue;
    });
    return config;
  }
