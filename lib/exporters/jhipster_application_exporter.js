...
function setUpApplicationStructure(application) {
  ...
  applicationToExport[GENERATOR_NAME].blueprints = application.config.blueprints;
...
function setUpArrayOptions(application) {
  application[GENERATOR_NAME].blueprints = Array.from(application[GENERATOR_NAME].blueprints).map(blueprintName => ({
    name: blueprintName,
    version: 'latest'
  }));
