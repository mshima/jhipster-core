 ...
 */
function convertToJDL(directory = '.', output = 'app.jdl', options = {}) {
  const { definitions } = options;
  let jdlObject;
  if (doesFileExist(path.join(directory, '.yo-rc.json'))) {
    jdlObject = getJDLObjectFromSingleApplication(directory, undefined, options);
  } else {
    try {
      jdlObject = getJDLObjectFromMultipleApplications(directory, options);
    } catch (error) {
      return;
    }
  }
  exportJDLObject(jdlObject, path.join(directory, output), definitions);
}

function getJDLObjectFromMultipleApplications(directory, options) {
  ...
  subDirectories.forEach(subDirectory => {
    jdlObject = getJDLObjectFromSingleApplication(path.join(directory, subDirectory), jdlObject, options);
  });
  return jdlObject;
}

function getJDLObjectFromSingleApplication(directory, existingJDLObject = new JDLObject(), options = {}) {
  ...
  const jdlApplication = convertApplicationToJDL({ application: cleanedYoRcFileContent, options });
