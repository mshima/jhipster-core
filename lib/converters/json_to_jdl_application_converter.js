...
function convertApplicationsToJDL({ applications, jdl } = {}, options = {}) {
  const jsonApplications = applications || [];
  const jdlObject = jdl || new JDLObject();
  jsonApplications.forEach(application => {
    const convertedApplication = convertApplicationToJDL({ application }, options);
    jdlObject.addApplication(convertedApplication);
  });
  return jdlObject;
}

function convertApplicationToJDL({ application } = {}, options = {}) {
  return createJDLApplication(application[GENERATOR_NAME], options);
}
