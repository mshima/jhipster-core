...
const { OptionNames, doesOptionExist } = require('./jhipster/application_options');
...
module.exports = class JDLApplicationConfiguration {
  ...
  toString(indent = 0, exportOptions) {
    ...
    const config = getFormattedConfigOptionsString(this.options, spaceBeforeOption, exportOptions);
    return `${spaceBeforeConfigKeyword}config {
${config}
...
function getFormattedConfigOptionsString(options, indent, exportOptions) {
  const filteredOptions = filterOutUnwantedOptions(options, exportOptions);
...
function filterOutUnwantedOptions(options, exportOptions = {}) {
  const { exportUnregisteredConfigs } = exportOptions;
  options = { ...options };
  options = filterOutOptionsWithoutValues(options);
  options = filterOutOptionsThatShouldNotBeExported(options);
  if (!exportUnregisteredConfigs) {
    options = filterOutGenericOptions(options);
  }
  return options;
}
...
function filterOutGenericOptions(options) {
  Object.values(options).forEach(option => {
    const shouldBeExported = doesOptionExist(option.getName(), option.getValue());
    if (!shouldBeExported) {
      logger.debug(`Unrecognized application option name and value: ${option.getName()} and ${option.getValue()}`);
      delete options[option.getName()];
    }
  });
  return options;
}
