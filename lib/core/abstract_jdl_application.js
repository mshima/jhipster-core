...
function generateConfigObject(passedConfig) {
  ...
  Object.keys(passedConfig).forEach(option => {
    ...
    if (Array.isArray(value) && ['languages', 'testFrameworks', 'blueprints'].includes(option)) {
...
function stringifyOptionValue(name, value) {
  if (['languages', 'testFrameworks', 'blueprints'].includes(name)) {
    if (value.size === 0) {
      return ' []';
    }
    return ` [${join(value, ', ')}]`;
...
function defaults() {
  return {
    blueprints: ApplicationOptions.blueprints,
