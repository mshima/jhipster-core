...
class JDLObject {
  ...
  toString(options) {
    ...
    if (this.getApplicationQuantity() !== 0) {
      string += `${applicationsToString(this.applications, options)}\n`;
    }
...
function applicationsToString(applications, options) {
  let string = '';
  Object.keys(applications).forEach(applicationName => {
    string += `${applications[applicationName].toString(options)}\n`;
  });
  return string;
}
